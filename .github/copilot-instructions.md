...
**Example:**
```go
// Package xerrs provides utilities for error handling.
package xerrs
```
...
| ----------- | ----------- | ------- |
| `{project}` | Project/repo name | `go-xerrs` |
| `{package}` | Go package name | `xerrs` |
| `{packagename}` | Package name in code | `xerrs` |
